...
# Model 1: exp rho t            (2 parameters: Y0, rho, not including sigma)
# Model 2: exp t(alpha - k)     (3 parameters: Y0, alpha, K, not including sigma)
# Model 3: Both together.       (5 parameters: Y0, pi, rho, alpha, K, not including sigma)
...
for training_instance_id in range(N_patients):
    ...
    this_estimate = estimate_drug_response_parameters_any_model(this_patient, lb_1, ub_1) #, sigma_noise_std=1, N_iterations=N_iter)
    ...
    this_estimate = estimate_drug_response_parameters_any_model(this_patient, lb_2, ub_2) #, sigma_noise_std=1, N_iterations=N_iter)
    ...
    this_estimate = estimate_drug_response_parameters_any_model(this_patient, lb_3, ub_3) #, sigma_noise_std=1, N_iterations=N_iter)
    ...
    # BIC values
    bic_values_1.append(len(lb_1)*np.log(N_observations_this_period) + negative_loglikelihoods_1[training_instance_id])
    bic_values_2.append(len(lb_2)*np.log(N_observations_this_period) + negative_loglikelihoods_2[training_instance_id])
    bic_values_3.append(len(lb_3)*np.log(N_observations_this_period) + negative_loglikelihoods_3[training_instance_id])
    print("BIC")
...
plt.tight_layout()
plt.savefig("./plots/simulation_plots/model_comparison_sigma_estimates_sigma_"+str(observation_std_m_protein)+"_"+str(N_iter)+"_iterations.png")
plt.show()
plt.close()
...
maxval = max(max(negative_loglikelihoods_1[0:int(N_patients/2)]), max(negative_loglikelihoods_2[0:int(N_patients/2)]), max(negative_loglikelihoods_3[0:int(N_patients/2)]))
minval = min(min(negative_loglikelihoods_1[0:int(N_patients/2)]), min(negative_loglikelihoods_2[0:int(N_patients/2)]), min(negative_loglikelihoods_3[0:int(N_patients/2)]))
diff = maxval - minval
...
maxval = max(max(negative_loglikelihoods_1[int(N_patients/2):N_patients]), max(negative_loglikelihoods_2[int(N_patients/2):N_patients]), max(negative_loglikelihoods_3[int(N_patients/2):N_patients]))
minval = min(min(negative_loglikelihoods_1[int(N_patients/2):N_patients]), min(negative_loglikelihoods_2[int(N_patients/2):N_patients]), min(negative_loglikelihoods_3[int(N_patients/2):N_patients]))
diff = maxval - minval
maxval = maxval + 0.1*diff
minval = minval - 0.1*diff
...
plt.tight_layout()
plt.savefig("./plots/simulation_plots/model_comparison_loglikelihood_sigma_"+str(observation_std_m_protein)+".png")
#plt.show()
plt.close()

maxval = max(max(bic_values_1[0:int(N_patients/2)]), max(bic_values_2[0:int(N_patients/2)]), max(bic_values_3[0:int(N_patients/2)]))
minval = min(min(bic_values_1[0:int(N_patients/2)]), min(bic_values_2[0:int(N_patients/2)]), min(bic_values_3[0:int(N_patients/2)]))
diff = maxval - minval
maxval = maxval + 0.1*diff
minval = minval - 0.1*diff
...
maxval = max(max(bic_values_1[int(N_patients/2):N_patients]), max(bic_values_2[int(N_patients/2):N_patients]), max(bic_values_3[int(N_patients/2):N_patients]))
minval = min(min(bic_values_1[int(N_patients/2):N_patients]), min(bic_values_2[int(N_patients/2):N_patients]), min(bic_values_3[int(N_patients/2):N_patients]))
diff = maxval - minval
maxval = maxval + 0.1*diff
minval = minval - 0.1*diff
...
plt.suptitle("BIC")
plt.tight_layout()
plt.savefig("./plots/simulation_plots/model_comparison_bic_values_sigma_"+str(observation_std_m_protein)+".png")
plt.show()
plt.close()
