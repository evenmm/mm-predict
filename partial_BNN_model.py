...
def partial_BNN_model(map_estimate, X, patient_dictionary, name, psi_prior="lognormal", MODEL_RANDOM_EFFECTS=True, FUNNEL_REPARAMETRIZATION=False, FUNNEL_WEIGHTS = False, WEIGHT_PRIOR = "symmetry_fix", SAVING=False, n_hidden = 3, net_list=["pi", "rho_r", "rho_s"], empirical_mean_alpha=[]):
    ...
    with pm.Model(coords={"predictors": X_not_transformed.columns.values}) as neural_net_model:
        ...
        if "rho_s" not in net_list:
            rho_s_population_logscale = pm.Deterministic("rho_s_population_logscale", np.repeat(alpha[0], N_patients))
        else: 
            ...
            rho_s_population_logscale = pm.Deterministic("rho_s_population_logscale", alpha[0] + act_out_rho_s)

        if "rho_r" not in net_list:
            rho_r_population_logscale = pm.Deterministic("rho_r_population_logscale", np.repeat(alpha[1], N_patients))
        else: 
            if FUNNEL_WEIGHTS == True: # Funnel reparametrized weights: 
                # Weights input to 1st layer
            ...
            rho_r_population_logscale = pm.Deterministic("rho_r_population_logscale", alpha[1] + act_out_rho_r)

        if "pi" not in net_list:
            pi_population_logscale = pm.Deterministic("pi_population_logscale", np.repeat(alpha[2], N_patients))
        else: 
            if FUNNEL_WEIGHTS == True: # Funnel reparametrized weights: 
                # Weights input to 1st layer
            ...
            pi_population_logscale = pm.Deterministic("pi_population_logscale", alpha[2] + act_out_pi_r)
        ...
        if MODEL_RANDOM_EFFECTS:
            if FUNNEL_REPARAMETRIZATION == True: 
                ...
                theta_rho_s = pm.Deterministic("theta_rho_s", (rho_s_population_logscale + theta_rho_s_offset * omega[0]))
                theta_rho_r = pm.Deterministic("theta_rho_r", (rho_r_population_logscale + theta_rho_r_offset * omega[1]))
                theta_pi_r  = pm.Deterministic("theta_pi_r",  (pi_population_logscale + theta_pi_r_offset  * omega[2]))
            else:
                # Original
                theta_rho_s = pm.Normal("theta_rho_s", mu=rho_s_population_logscale, sigma=omega[0]) # Individual random intercepts in theta to confound effects of X
                theta_rho_r = pm.Normal("theta_rho_r", mu=rho_r_population_logscale, sigma=omega[1]) # Individual random intercepts in theta to confound effects of X
                theta_pi_r  = pm.Normal("theta_pi_r",  mu=pi_population_logscale,  sigma=omega[2]) # Individual random intercepts in theta to confound effects of X
        else: 
            theta_rho_s = pm.Deterministic("theta_rho_s", rho_s_population_logscale)
            theta_rho_r = pm.Deterministic("theta_rho_r", rho_r_population_logscale)
            theta_pi_r  = pm.Deterministic("theta_pi_r",  pi_population_logscale)
        ...
        rho_s = pm.Deterministic("rho_s", -pm.math.exp(theta_rho_s))
        rho_r = pm.Deterministic("rho_r", pm.math.exp(theta_rho_r))
        pi_r  = pm.Deterministic("pi_r", 1/(1+pm.math.exp(-theta_pi_r)))
