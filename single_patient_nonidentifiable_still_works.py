...
true_alpha = np.array([theta_expected_rho_s_population, theta_expected_rho_r_population, theta_expected_pi_r_population])
...
with single_patient_model:
    ...
    theta_expected_rho_s = alpha[0] + X*beta[0] #pm.Normal("theta_expected_rho_s", mu=X*true_beta[0], sigma=1)
    theta_expected_rho_r = alpha[1] + X*beta[1] #pm.Normal("theta_expected_rho_r", mu=X*true_beta[1], sigma=1)
    theta_expected_pi_r  = alpha[2] + X*beta[2] #pm.Normal("theta_expected_pi_r",  mu=X*true_beta[2], sigma=1)
    ...
    psi = pm.Normal("psi", mu=psi_population, sigma=10)
