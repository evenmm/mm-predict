...
name = "simdata_"+model_name+"_"+str(script_index)+"_M_"+str(M_number_of_measurements)+"_P_"+str(P)+"_N_pax_"+str(N_patients)+"_N_sampl_"+str(N_samples)+"_N_tune_"+str(N_tuning)+"_FUNNEL_"+str(FUNNEL_REPARAMETRIZATION)+"_RNDM_EFFECTS_"+str(RANDOM_EFFECTS)
print("Running "+name)
...
with lin_model:
    if ADADELTA: 
        ...
        idata = pm.sample(draws=N_samples, tune=N_tuning, init="advi+adapt_diag", chains=n_chains, n_init=60000, random_seed=42, target_accept=target_accept)

print("Done sampling")

picklefile = open(SAVEDIR+'idata_'+name, 'wb')
...
recurrence_or_not, p_recurrence = pfs_auc(evaluation_time, patient_dictionary_test, N_patients_test, idata, X_test, model_name, MODEL_RANDOM_EFFECTS, CI_with_obs_noise, SAVEDIR, name)
